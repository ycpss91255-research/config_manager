...
**這張表解釋的是「為什麼有這幾項」，不是「有哪幾項」。** 完整的清單是
`./script/test.sh -h` 的輸出——那一份不會落後於實際跑了什麼。
...
- **不要重建設計文件。** 直接使用；有新決策就在 `doc/adr/` 加新號，接在現有的最大編號之後。
...
issue 依能力矩陣逐項拆分，各自帶驗收條件（§0.7：驗收條件屬 issue 的 acceptance criteria）。

**動工前先關掉 [#1](https://github.com/ycpss91255-research/config_manager/issues/1)**——
逐項確認 `doc/TEST-PLAN.md` 已列出的每個測試介面與每條驗收旅程。沒有經過確認的測試介面，不寫測試。

`v1.0.0` 不設在本規劃內，待實際運行一段時間後由維護者自行標記。

## 目前狀態

**每個模組落地到哪裡、由哪個測試介面觀察，看 `doc/TEST-PLAN.md` 的「覆蓋率審計 →
模組 → 測試介面」表。** 那張表有「狀態」欄，逐列寫著那個模組落地了沒有，
而它是這件事的唯一 owner。

這裡不放第二份。上一份放在這裡的清單過期了——它說 `io/` 與 `api/` 尚未開始，
而那兩層當時都已經有模組合併進來（#98）。兩份寫著同一件事，沒被讀的那份會過期
（ADR-00000024），而讀者無從分辨自己讀到的是哪一份。
