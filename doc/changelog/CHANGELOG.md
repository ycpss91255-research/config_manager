...
- `script/acceptance.sh <milestone>` 與 `doc/acceptance-checkpoints.toml`：設計 §8.2
  的驗收檢查點與驗收它的規格之間有了一份對照表，而「那一版通過了沒有」由一次執行
  回答，不由散文回答。一條檢查點對不到任何規格時判定為**未涵蓋**，未涵蓋讓整份
  報表非零結束——把「沒有人在驗」讀成「驗過了」，正是這個 repo 已經抓到八次的那個
  形狀。對照表指到不存在的規格是失敗而不是安靜跳過，理由與 `test.sh` 的「缺工具
  不得靜默通過」相同（#148）。

  **第一次執行就是紅的，那是刻意的順序。** v0.1.0 的五條檢查點：4 通過／0 未通過／
  1 未涵蓋，結束碼 1。未涵蓋的是檢查點 3「修改清單檔後寫出，註解與欄位順序完整
  保留」——當時 `core/config_list.dump` 還不支援改動與移除既有條目，沒有任何規格
  驗得了那件事。**一份從沒報過紅的報表，證明不了它會報紅。**

  **然後它轉綠了。** #151（來由 #149）補上改動、移除、以及三者同時發生的路徑，
  對照表因此對得上那一條，同一支腳本回報 5 通過／0 未通過／0 未涵蓋、結束碼 0
  ——**腳本一行都沒有改**。當初若把「未改動時逐位元組相同」那兩條填進去換一個
  綠燈，這次轉綠就什麼都不代表了。

- `script/release.sh` 與 `.github/workflows/release.yaml`：驗收報表隨 `vX.Y.Z-rcN`
  發布。推上 tag 時 CI 在**那個 tag 指的 commit 上**重跑一次報表，判定寫進 release
  的標題與 notes 第一行，同一份內容作為附加檔案——本機跑得出來的報表只證明「某個人
  在某個時間點跑過」，release 上的報表才複驗得了（#150）。

  **紅的 rc 照發，紅的正式 tag 發不出去。** rc 的用途是記錄「現在走到哪」，擋下紅色
  的 rc 等於讓 N 無法遞增；而非 rc 的 `vX.Y.Z` 只有報表全數通過才建得起來，擋下時
  逐條指名沒過的檢查點。閘門查的是「這個 commit 的報表」而不是「有沒有一個綠的 rc」
  ——後者擋不住「rc2 是綠的，之後又進了三個 commit 才打正式 tag」。

  rc 的編號由既有 tag 推導，而且**推上來的 rc 若不是推導出來的那一個會被擋下**：
  少了後面這一半，「由工具推導」就只是一句建議（§0.4）。

- `acceptance` job 接進 CI，跑在 main 的 push 上而不是每個 pull request 上。它會把
  對照表指到的規格再跑一次（幾乎是整組），接成 PR 的必要檢查等於 CI 時間翻倍換同一
  批規格的第二個答案；它多出來的訊號是**對照表與規格之間的對應**，而那個訊號不能等
  到打 tag 那天才出現——對照表指錯時 `acceptance.sh` 回 2，而 `release.sh` 對 2 的
  處置是不建立 release（#150）。

- `script/lint_coverage_audit.sh`：`doc/TEST-PLAN.md` 的「模組 → 測試介面」表與
  `src/`、`script/` 的現況對不起來就擋。那張表自己立了「新增一個模組或一支腳本時，
  這裡要一起加一列」這條規則，而在這支 lint 之前沒有工具擋著它——依 §0.4 它先前
  並不存在，而它也確實漂了兩輪（#104 加了模組沒加列；#113 補齊之後 #150 的
  `release.sh` 又讓腳本數落後）。五條規則：表讀不讀得出來、有沒有漏列、對不到檔案
  的列有沒有宣告未落地、**宣告未落地的列是不是真的還沒落地**，以及這一節有沒有
  手抄可推導的數量。最後一條把那些數字從文件裡拿掉——可推導的東西在被閱讀時計算，
  不存一份要人維持一致的副本（不變式 9）。留在表上的仍然是產生器做不出來的那一半：
  哪個模組落在哪個測試介面、以及為什麼（#117）。
- `script/lint_derived.sh`：文件裡抄一份可從樹推導出來的東西就擋。`README.md` 的
  「目前狀態」把 `doc/TEST-PLAN.md` 的測試介面清單抄了一份然後過期——它說 `io/` 與
  `api/` 尚未開始，而那兩層當時都已經有模組合併進來。同一份檔案裡另外四處同類的
  數字，其中三處也已經不對（ADR 份數、下一個 ADR 編號、測試介面個數、issue 張數）。
  markdown 沒辦法在被閱讀時計算，所以不變式 9 在這裡只有一個實作方式：**把副本拿掉，
  指向來源**——而「拿掉」要有東西擋著，否則它只是這一次的清理（#98）。
...
- **測試與文件（不改變行為，但改變了「哪些行為有證據」）。** 四支守門腳本補完
  ...
  `test/pytest/system/test_api.py` 的每則規格自己排前置狀態，不再靠同檔的鄰居
  先跑過（#153）。**這一條是 `script/acceptance.sh` 逐條執行才照出來的**：
  `pytest test/pytest` 永遠整檔收集，順序相依在那之下看不見。修之前有兩則靠順序
  ——CLI 那則靠鄰居**先**跑（單獨執行紅），空清單那則靠鄰居**後**跑（單獨執行綠，
  順序對調才紅）。第二則是逐條檢查驗收條件時才浮出來的，issue 上只記了第一則。
  `doc/acceptance-checkpoints.toml` 的 v0.1.0 檢查點 5 隨之從整個 `test_api.py`
  改回指那一條規格。
