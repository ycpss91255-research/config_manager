...
| `hadolint` | Dockerfile | — |
| `script/coverage_gate.sh` | 行覆蓋率——**四個資料夾各自計算、各自擋** | `core`／`io`／`api`／`web` |

閾值不寫在文件裡，寫在 `pyproject.toml`——無法自動檢查的規範等於不存在。
調整閾值須修改設定檔並在 PR 中說明理由，使閾值本身成為可見的決策。

**`mypy` 與覆蓋率原本的範圍都只有 `core/`**，於是 `io/`／`api/` 有整合與系統層的規格，
卻沒有任何數字會在它們的分支未被執行時轉紅；`web/` 連規格都沒有。#97 把兩者都補上：
`--strict` 涵蓋整個套件，覆蓋率四個資料夾各自 85（實測 97.29／88.70／99.28／98.82）。

**四個門檻是四個判定，不是一個總數。** 一個門檻同時守四個標準不同的區域，結果是守住最鬆的
那個；四條不同的線則是把現況封成標準。同一條線、分開算、分開擋，指名的只有真的轉紅的那一層
——由 `test/bats/unit/coverage_gate.bats` 逐條驗證。
