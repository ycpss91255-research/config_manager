...
# ## 突變檢查（每條規則各拿掉一次，#98）
#
# `_rules` 的每一列各算一條規則——那張表是資料，一列就是一種被擋下的寫法。
#
# | 突變 | 轉紅的規格 |
# |---|---|
# | M1 拿掉「ADR 份數」那一列 | 2、3 |
# | M2 拿掉「下一個 ADR 編號」那一列 | 4 |
# | M3 拿掉「測試介面個數」那一列 | 6 |
# | M4 拿掉「驗收旅程個數」那一列 | 7 |
# | M5 拿掉「issue 張數」那一列 | 8 |
# | M6 R3 的代號比對改成永不命中 | 9、10 |
# | M7 檔案不在時改成安靜通過 | 12 |
#
# 七個突變各套用一次再還原，還原後與突變前逐位元組相同。沒有一個突變是靜默通過的。
