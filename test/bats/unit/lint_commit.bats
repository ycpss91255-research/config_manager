...
@test "格式錯誤時給的示範是中文陳述句，不是叫人寫英文" {
  # 守門腳本最重要的產出是它的訊息。ADR-00000028 之後主旨是中文，中文沒有大小寫，
  # 「lowercase」這條規則在 #70 已確認不存在——但訊息還在教人遵守它。
  # 一條不存在的規則仍出現在錯誤訊息裡，是訊息與行為不符（#106）。
  commit_subject "清單檔載入時攔截未知欄位"

  run "${LINT}" "${BASE}"
  [ "${status}" -ne 0 ]
  [[ "${output}" != *"lowercase"* ]]
  # 示範本身要是合規的：一則過得了這支 lint 自己的主旨。
  [[ "${output}" == *"feat(core): 清單檔載入時攔截未知欄位"* ]]
}
