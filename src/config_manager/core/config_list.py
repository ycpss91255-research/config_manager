...
def dump(config_list: ConfigList, original: str) -> str:
    ...
    **寫出之前先自我驗證**，與 `load` 同一組檢查（#181）。先前這一步只掛在 `load`
    上，於是無效的條目寫得進清單檔，而錯誤要到下一次啟動才爆——那時擋的是容器啟動，
    不是造成它的那次寫入，訊息也指不回真正的成因。把「記得先檢查」留給呼叫端，等於
    一條沒有東西擋著的約定（§0.4）；不變式 2 要的是**契約在做實事之前先自我驗證**。

    檢查不通過時丟出與 `load` 相同的具名例外，且**不產生任何輸出**。
    """
    _check_integrity(config_list)
